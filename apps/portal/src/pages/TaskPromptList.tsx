    ...
    },
    {
      ...
      cell: (tp) => (
        <div className="flex items-center gap-1 justify-end">
          <Link to={`/task-prompts/${tp.id}`} onClick={(e) => e.stopPropagation()}>
            <Button variant="ghost" size="icon" className="h-8 w-8">
              <Eye className="h-4 w-4" />
            </Button>
          </Link>
          <Link to={`/runs?taskPromptId=${encodeURIComponent(tp.id)}`} onClick={(e) => e.stopPropagation()}>
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <DataTable
          ...
          activeId={activeId}
          onRowClick={(tp) =>
            navigate({ pathname: `/task-prompts/${tp.id}/preview`, search: window.location.search })
          }
          selection={{
            selectedIds,
            onToggle: toggleRow,
