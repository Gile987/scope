...
export function TokenList() {
  ...
  const filteredTokens = useMemo(() => {
    ...
    return activeTokens.filter((t) => {
      ...
      if (q) {
        const blob = `${t.id} ${t.comment ?? ""} ${t.type} ${(t.capabilities ?? []).join(" ")}`.toLowerCase();
      ...
      cell: (t) => (
        <div className="flex items-center justify-end gap-1">
          <Button
            ...
            onClick={(e) => { e.stopPropagation(); validateMutation.mutate(t.id); }}
            title="Validate key"
          >
        ...
        >
          <div className="flex flex-col gap-3">
            ...
            <DataTable
              ...
              activeId={activeId}
              onRowClick={(t) =>
                navigate({ pathname: `/secrets/keys/${t.id}/preview`, search: window.location.search })
              }
              selection={{
                selectedIds,
                onToggle: toggleRow,
