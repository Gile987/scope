...
promptFeature
  ...
  .option("--type <type>", "Filter by feature type ('task' or 'agents.md')")
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const displayFields: DisplayField[] = [
        ...
        { key: 'type', label: 'Type', formatter: (f: any) => f.type ?? 'task',
          tableFormatter: (f: any) => value(f.type ?? 'task') },
...
promptFeature
  ...
  .option("--type <type>", "Feature type ('task' default, or 'agents.md')")
  .option("-u, --url <url>", "API base URL", getDefaultApiUrl())
  .action(async (options) => {
    try {
