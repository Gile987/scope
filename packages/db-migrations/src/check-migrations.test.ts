...
describe("checkMigrations", () => {
  ...
  it("ignores extra applied migrations not in the required list", async () => {
    ...
    expect(result.ready).toBe(true);
    expect(result.pending).toEqual([]);
    expect(result.totalApplied).toBe(19);
  });
