...
Portal and API authentication follow
[`docs/architecture/auth-rbac.md`](docs/architecture/auth-rbac.md). Local Docker
development uses the same real Microsoft Entra ID flow as other environments,
or the local Entra-compatible emulator configured as a normal IdP. There is no
auth bypass mode.

For Docker Compose, put these values in `.env.local` (not `.env`, which is
regenerated by `worktree-env`). Compose maps the shared `AUTH_*` values into the
Portal's Vite-exposed `VITE_AUTH_*` variables:

| Shared env var | Portal Vite env var | Purpose |
| --- | --- | --- |
| `AUTH_AUTHORITY` | `VITE_AUTH_AUTHORITY` | Entra authority, e.g. `https://login.microsoftonline.com/common` |
| `AUTH_CLIENT_ID` | `VITE_AUTH_CLIENT_ID` | Public client / SPA app registration client ID |
| `AUTH_SCOPES` | `VITE_AUTH_SCOPES` | Comma-separated scopes requested by MSAL |
| `AUTH_API_CLIENT_ID` | `VITE_AUTH_AUDIENCE` | API app registration audience expected by the API |
| `AUTH_KNOWN_AUTHORITIES` | `VITE_AUTH_KNOWN_AUTHORITIES` | Optional comma-separated trusted authority hosts for local/non-public-cloud authorities |
...
Microsoft Entra authority, typically
`https://login.microsoftonline.com/common` for multi-tenant local development.

### AUTH_API_CLIENT_ID
**Required**
**Type:** string

API app registration client/application ID. The API validates access-token
audience against this value; the Portal receives it as `VITE_AUTH_AUDIENCE`.

### AUTH_CLIENT_ID
**Required**
**Type:** string

Public client / SPA app registration client ID used by MSAL in the Portal.

### AUTH_SCOPES
**Required**
**Type:** comma-separated string

Scopes requested by the Portal, for example
`api://<api-app-id>/access_as_user`.

### AUTH_KNOWN_AUTHORITIES
**Required for Entra-compatible local emulators**
**Type:** comma-separated host[:port] list

Trusted authority hosts passed to MSAL `knownAuthorities`. Leave unset for
Microsoft's public Entra cloud. Set to `localhost:<ENTRA_LOCAL_PORT>` when using
the bundled Entra Local service.

### ENTRA_LOCAL_PORT
**Default:** `8500`
**Type:** integer

Host port for the local Entra ID emulator. `worktree-env` offsets this port per
worktree, just like `PORTAL_PORT`.
...
emulator's seeded app registrations:

| Purpose | Value |
| --- | --- |
| Tenant | `11111111-1111-1111-1111-111111111111` |
| SPA client ID | `cccccccc-0000-0000-0000-000000000004` |
| API audience/client ID | `cccccccc-0000-0000-0000-000000000005` |
| Scope | `api://cccccccc-0000-0000-0000-000000000005/access_as_user` |

The `entra-local-init` Compose job registers
`http://localhost:${PORTAL_PORT}` as a redirect URI on the seeded SPA app so
worktree-specific Portal ports can sign in without manual emulator setup.

### AUTH_BOOTSTRAP_ADMINS
**Required to promote initial admins**
**Type:** comma-separated identity tuple list

Verified Entra subjects that should be promoted to admin, formatted as
`entra:<tenant-id>/<object-id>`.

### AUTH_BOOTSTRAP_TENANTS
**Required with `AUTH_BOOTSTRAP_ADMINS`**
**Type:** comma-separated tenant ID list

Tenant allowlist for bootstrap admin promotion.
