...
pub trait ProxyPlugin: Send + Sync {
    ...
    /// (e.g., `{"redactCredentials": true}` for the HAR plugin).
...
- **`on_session_start(settings)`** — reads `settings["redactCredentials"]` (default `true`) and stores it for the session. Creates a temp file (`{har_dir}/.session-{session_id}.jsonl`).
- **`on_exchange`** — serializes the `HarEntry` as a single JSON line and appends it to the session's temp file. **Redacts sensitive headers** (`authorization`, `x-github-token`, `x-api-key`, `cookie`, `set-cookie`) at write time when `redactCredentials` is `true`. Secrets never touch disk.
...
1. **New `startSession(pluginSettings)` method** — `POST /session/start` with per-plugin settings (e.g., `{ har: { redactCredentials: true } }`). Replaces `startRecording()`.
