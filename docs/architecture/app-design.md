...
- **`components/ProjectGate.tsx`** guards scoped routes: when no project is selected it renders a first-run pick/create screen (`ProjectFirstRunView`) instead of firing a scoped request that would 400. Point-read detail routes (resolve by `_id`) and unscoped areas (agents, models, secrets, admin, `/projects`) stay ungated, so a run detail URL is always viewable without a prior selection. Such a directly-opened **run** URL then auto-scopes the app to the run's own project via **`hooks/useAutoScopeProject.ts`** (used by `pages/RunDetail.tsx`): once the ungated point-read resolves it selects `run.projectId` — always, even over a different existing selection, but only **once per run id** (a ref guard prevents an update loop and avoids fighting `ProjectSwitcher`'s self-heal, while still respecting a later manual switch). It preserves the run page's own unscoped, id-keyed queries (`run`, `run-attempts`, `run-reports`, `profile`, `task-prompt`) across the switch to avoid a reload flash, while the scoped `report-templates` query refetches under the new project. The index route `/` is served by **`components/HomeRoute.tsx`**, the unscoped **home**: on entry it clears any active project (`useSelectProject(undefined)`, which also resets scoped query caches) and renders the picker, then forwards to `/statistics` once the user picks a project. Reaching `/` by any means (the logo, a typed URL, the back button, a bookmark) therefore de-scopes; there is no default project.
