...
- Whenever the coding agent captured tool calls in an iteration — **any gate,
  including Select** — the judge can inspect that captured tool-call history via
  the `read_tool_outputs` / `get_tool_output` tools, not just the workspace files
  (availability is gate-agnostic; see `buildEvidenceGuidance`). The judge can also
  read the coding agent's own response for the iteration under evaluation via the
