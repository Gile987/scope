...
1. User selects a base `profileId` (optionally pinned via `profileId@version`) and a `profileVariations[]` array of additional profiles to compare against. Each variation entry is `{ profileId: "<id>" | "<id>@<version>", label?: string }`.
