...
- **`components/ProjectSwitcher.tsx`** is the header control (beside `ThemeToggle`) that lists projects, switches the active one — resetting all scoped queries via `hooks/useSelectProject.ts` (which can optionally **preserve** caller-named unscoped query-key roots), since query keys don't embed `projectId` — and offers inline create + a link to `/projects`. Its presentational `ProjectSwitcherView` is story/play-tested. On entity **detail pages** it renders in a non-interactive **locked** mode (static folder icon + project name, no chevron/dropdown, `aria-disabled`) — see `LockProjectSwitcher` below.
- **`components/ProjectGate.tsx`** guards scoped routes: when no project is selected it renders a first-run pick/create screen (`ProjectFirstRunView`) instead of firing a scoped request that would 400. Point-read detail routes (resolve by `_id`) and unscoped areas (agents, models, secrets, admin, `/projects`) stay ungated, so a run detail URL is always viewable without a prior selection. Such directly-opened point-read detail URLs then auto-scope the app to the resource's own project via **`hooks/useAutoScopeProject.ts`** — used by `pages/RunDetail.tsx`, `ReportDetail.tsx`, `InsightDetail.tsx`, `CodebaseDetail.tsx`, `ProfileDetail.tsx`, and `TaskPromptDetail.tsx`: once the ungated point-read resolves it selects the resource's `projectId` — always, even over a different existing selection, but only **once per resource id** (a ref guard prevents an update loop and avoids fighting `ProjectSwitcher`'s self-heal, while still respecting a later manual switch). Each page preserves its own unscoped, id-keyed queries across the switch to avoid a reload flash (e.g. RunDetail keeps `run`, `run-attempts`, `run-reports`, `profile`, `task-prompt`), while genuinely scoped queries (e.g. the run page's `report-templates`) refetch under the new project. This works only for detail routes whose GET resolves by a **global `_id`** and returns `projectId`; routes keyed by a **per-project slug** with a project-scoped GET (`/criteria/:id`, `/mcp-servers/:slug`, `/reports/templates/:id`, `/skills/*`, `/extensions/:id`) can't derive a single project from the URL and stay on the current selection. The index route `/` is served by **`components/HomeRoute.tsx`**, the unscoped **home**: on entry it clears any active project (`useSelectProject(undefined)`, which also resets scoped query caches) and renders the picker, then forwards to `/statistics` once the user picks a project. Reaching `/` by any means (the logo, a typed URL, the back button, a bookmark) therefore de-scopes; there is no default project.
- **`components/LockProjectSwitcher.tsx`** + **`contexts/ProjectSwitcherLockContext.tsx`** are the inverse of `ProjectGate`, for **detail** pages: switching projects from a single-entity page is meaningless (the entity fixes its project, and after auto-scope the switcher already shows that project), so `App.tsx` wraps each project-scoped entity **detail** route element (`/runs/:id/:tab?`, `/reports/:id`, `/reports/templates/:id`, `/insights/:id`, `/criteria/:id`, `/task-prompts/:id`, `/mcp-servers/:slug`, `/skills/*`, `/codebases/:id` (+`/revisions/:revisionId`), `/extensions/:id`, `/profiles/:profileId` (+`/v/:version`)) in `<LockProjectSwitcher>` (placed **inside** any `FeatureRoute` so the lock only engages when the detail actually renders). On mount it sets a boolean `locked` flag in the lock context; on unmount it clears it — a plain boolean suffices because the `<Routes>` component API mounts exactly one matched element at a time, and React runs effect cleanups before setups on a detail→detail navigation, so there is no intermediate unlocked render. `ProjectSwitcher` reads the flag and renders `ProjectSwitcherView` in its non-interactive locked mode. The lock is **presentation-only** — it never blocks programmatic selection, so `useAutoScopeProject`, the switcher self-heal, and `HomeRoute` all still call `selectProject(...)`. As a safety valve the container only actually locks once a project is resolved (`locked && selectedProjectId`), so a detail page opened cold with no selection (a slug/`:id` route that can't auto-scope) stays interactive and the user is never stranded. **Not** wrapped (switcher stays interactive): list pages, create/`new` pages, `/criteria/graph|mdp`, `/statistics`, `/projects`, `/admin`, the `/` home, list-embedded preview panels (`:id/preview`, and the nested `/prompt-features/:id`), and the global detail pages `/agents/:id`, `/models/:id`, `/secrets/keys/:id`, `/secrets/accounts/:id`.
