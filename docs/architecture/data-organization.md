# Data Organization: Projects & Tags
...
proposes a first-class way to **organize** that data **within a single Kubernetes cluster** —
**starting with a single structure, the project** (a named container data is filed under) and
**phasing in tags** (lightweight cross-cutting labels).

This is the **data-organization layer** only. It defines the `projects` container plus the
`projectId` and `tags` fields, and how data is **filed, filtered, and grouped** by them.
**Access control — ownership, membership, groups, visibility, and authorization — is out of
scope and owned by [auth-rbac.md](auth-rbac.md).** The two are orthogonal layers: this design
populates the `projectId` field auth-rbac already reserved, and auth-rbac's access model can
later key off that field to enforce project-scoped visibility. This doc adds **no** permissions,
roles, ownership, or enforcement, and re-models **no** existing data.

> **Landing order — independent of auth-rbac.** auth-rbac.md is itself `Proposed`. Because this
> layer adds only organization (a container + two fields + filtering) and **no** access control,
...
That container — and the cross-cutting tags beside it — is what this document adds. Access
control stays entirely with auth-rbac; this layer only decides how data is *organized*, not who
may *see* it.
...
2. **Cross-cutting labels** ("tags") for organizing data along axes that don't fit a single
   container.
3. **Purely additive & non-breaking**: existing flat/global data keeps working unchanged; the
   feature is opt-in and reversible phase-by-phase.
4. **Clean composition with access control**: expose the `projectId` dimension that auth-rbac's
...
- **Access control — out of scope, owned by auth-rbac.** Ownership (`ownerId`/`ownerType`/
  ...
  **all** defined by [auth-rbac.md](auth-rbac.md), not here. This layer neither adds nor changes
  any of them; it only adds the `projectId`/`tags` organizing dimension they can compose with.
...
| `task-prompts` | Content-addressed prompts (`task` / `agents.md`) | Content-addressed building block |
| `mcp-servers` | MCP server configs | Durable |
| `codebases` / `codebase-revisions` | Source snapshots | Durable (codebase) / immutable (revision) |
| `reports` / `insights` | Judge output, derived from a run | Derived (follows parent) |
| `prompt-features` / `-extractions` | Extracted features | Content-addressed building block |
...
can slot in later. **This document is that layer**: it fills in `projectId`, adds a `tags` array,
and introduces the `projects` container. (auth-rbac's other reserved fields — `ownerType`,
`groupId`, `sharedWith` — belong to its access model and are **not** touched here.)

### Landing order (independent)

This design and auth-rbac are **orthogonal layers**. This layer adds only *organization* — a
container plus the `projectId`/`tags` fields plus filtering — and defines **no** identity,
ownership, visibility, or enforcement. So it has **no** dependency on auth-rbac in either
direction, and auth-rbac has none on it:

| Layer | Owned by | Needs caller identity? | Depends on the other? |
|-------|----------|------------------------|-----------------------|
| **Organization** (`projects`, `projectId`, `tags`; filing, `groupBy:"project"`, active-project filter) | **This doc** | No — filing and filtering only *narrow* result sets | No |
...
**Start with a single organizing structure — the Project — and phase in Tags.** This design uses
**two** primitives with clearly separated jobs, adopted in order. **Project** is the foundational
container: it ships first and satisfies the "file related work together / stop mixing everyone's
data into one list" goal on its own. **Tags** are an additive, cross-cutting layer phased in
afterward, behind its own phase, not blocking the first.

| Primitive | Job | First lands | Cardinality |
|-----------|-----|-------------|-------------|
| **Project** — _start here_ | Primary **organizing container**; the thing data is *filed under* and filtered/grouped by | **P1** | Each entity has **one** `projectId` |
| **Tag** — _phased in_ | **Cross-cutting**, many-to-many labels for filtering/organizing across projects | **P2** | An entity has **many** `tags` |
...
### Why start with Project (and phase in Tags)

- **Project alone satisfies the core goal, so it ships first.** A single `projectId` gives every
  entity a durable home and makes "show me only this workstream" a one-clause filter — with **no**
  dependency on tags or on the access layer. It is the MVP that resolves the "find my work / stop
  mixing everything together" pain in the first phase; tags are an additive enhancement that
  follows without re-work. This is the "start with one structure, phase the rest" spine of the
...
- **Tags recover the cross-cutting need without the cost.** "This run belongs to three efforts" is
  real, but modelling it as multi-project membership forces the singular field to become an array
  and complicates every filter. Tags give many-to-many organization at the price of one multikey
  index, and stack *on top of* the project filter.
- **Organization ≠ access, so this layer stays small.** By leaving ownership, membership, groups,
  and visibility to auth-rbac, this design reduces to a container + two fields + filtering —
  shippable in any order (see [Landing order](#landing-order-independent)).

Trade-offs and the rejected shapes (tags-only, many-to-many projects, nested projects) are in
...
Every [project-scoped](#which-entities-are-project-scoped) entity gains exactly **two** fields:

- `projectId?: string` — the project this entity is **filed under** (exactly one). Missing ⇒
  **coalesces to the Default project** (see [Migration](#migration)); after backfill every doc
  carries one, so no document is ever project-less.
- `tags?: string[]` — cross-cutting labels for filtering/organizing (no access effect).

This design adds **nothing else** to existing documents. auth-rbac separately adds `ownerId`,
`visibility`, and its other access fields to the same documents; the two field sets are disjoint
and independent (see [Landing order](#landing-order-independent)).

### Which entities are project-scoped

- **Project-scoped** (carry `projectId` + `tags`): runs (`requests`), profiles, criteria, personas,
...
- **Global building blocks** (stay unscoped): content-addressed / immutable dedup entities —
  `task-prompts`, `prompt-features`/`-extractions`, `skill-revisions`, `codebase-revisions`. A
  single content hash is referenced by runs across many projects, so filing it under one project
  is wrong. They are reached **through their parent entity** (e.g. a run), which does carry a
  `projectId`.
- **Admin-curated catalog** (agents, models, skills, extensions, report templates): remain a
  global catalog for now; whether any become project-scoped is an
  [open question](#open-questions).

### Shape decisions

- **One project per entity.** An entity is filed under a single project; cross-cutting grouping is
  **tags**, not multi-project filing. (Multi-project filing is an
...
Projects and tags are **organizational**, not access-control, constructs. They decide how data is
...
- **Tags are cross-cutting filters.** `tags` label an entity along axes that cut across projects
  (e.g. `regression`, `q3-eval`, `flaky`). They compose with the project filter (`projectId == X
  AND tags ∋ "regression"`) and carry no access meaning.
- **Building blocks are reached through their parent.** Global, content-addressed entities
  (`task-prompts`, etc.) have no `projectId`; they surface via a parent that does (e.g. a run).
...
> no enforcement yet); the project/tag filters still work. auth-rbac later inserts real enforcement
...
Until then, `projectId` and `tags` are **unenforced organizing dimensions**: present, indexed,
and filterable, but they never widen or restrict access on their own.
...
- **Create + migrate (canonical).** Create one **"Default"** project (`slug: "default"`), then
  **backfill `projectId: "default"`** onto all existing docs in the project-scoped collections.
  After it runs, every entity physically carries a `projectId` — there is no null/global bucket. The
...
- Entities accept `projectId` and `tags` on create/update; responses include them so clients can
  show project/tags and offer filtering.
...
- Per-entity **"Move to project"** action and a **tag editor**; project + tags shown on list rows
  and detail pages.
...
The sequence deliberately **starts with the single Project structure** (P0–P1) and **phases in**
Tags (P2), so the core organize-and-group goal ships before the additive label layer. Each phase is
independently shippable, reversible, and non-breaking; earlier phases change **no** behavior because
everything defaults into the Default project and unset `projectId` is treated as Default.
...
- **P0 — Schema + Default + backfill (invisible).** Add `projectId`/`tags` (optional, ignored by
...
- **P1 — Projects CRUD + context + Runs dimension.** `/api/v1/projects` CRUD; active-project context
  ...
  opt-in.
- **P2 — Tags.** Cross-cutting `tags` filtering across Portal/API/CLI.

> **Independent of auth-rbac.** No phase here needs an identity or access layer — every phase adds
> only organization (a container, two fields, filtering) and can ship **before, after, or
...
- **Catalog scoping.** Are admin-curated types (agents, models, skills, extensions, report
  templates) ever project-scoped, or always global?
...
- **Building blocks.** Do content-addressed entities (`task-prompts`, revisions) ever need their
  own `projectId`, or is reaching them through their parent always sufficient? (Recommendation:
  keep global; reach through parent.)
- **Tag governance.** Free-form tags vs a curated/validated tag vocabulary (per project or global).

---

## Alternatives considered

- **Tags only (no container).** Cheapest to build, but a flat tag namespace gives no durable "home"
  to file work under and no default scope for new data — every list still mixes everything together
  until you remember to filter. Kept as the *secondary* primitive, not the primary one.
- **Many-to-many project filing per entity.** More flexible ("this run is in three projects"), but
  ...
  "which project this belongs to." Rejected; **tags** cover the cross-cutting need at far lower cost.
